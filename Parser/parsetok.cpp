...
node * PyParser_ParseString(const char *s, grammar *g, int start, PerrDetail *err_ret)
{
    return PyParser_ParseStringFlagsFilename(s, NULL, g, start, err_ret, flags);
}

node * PyParser_ParseStringFlags(const char *s, grammar *g, int start, PerrDetail *err_ret, int flags)
{
    return PyParser_ParseStringFlagsFilename(s, NULL, g, start, err_ret, flags);
}
...
PyParser_ParseStringObject(const char *s, PyObject *filename,
                           ...
                           PerrDetail *err_ret, int *flags)
{
    if(err_ret->initialize(filename) < 0)
        return NULL;

    bool exec_input = (start == file_input);

    TokenState *p_token_state = (*flags & PyPARSE_IGNORE_COOKIE) ?
                                PyTokenizer_FromUTF8(s, exec_input) :
                                PyTokenizer_FromString(s, exec_input);

    if (p_token_state == NULL) 
    {
        if(PyErr_Occurred())
        {
            err_ret->setError(E_DECODE);
        }
        else
        {
            err_ret->setError(E_NOMEM);
        }
        return NULL;
    }

#ifndef PGEN
    Py_INCREF(err_ret->getFilename());
    err->setFilename(p_token_state->getFilename());
#endif
...
PerrDetail::PerrDetail():error(E_OK),lineno(0),offset(0),text(NULL),token(-1),expected(-1)
{
#ifndef PGEN
    filename = NULL;
#endif
...
int PerrDetail::initerr(PyObject *p_filename)
{
#ifndef PGEN
    if(p_filename)
    {
